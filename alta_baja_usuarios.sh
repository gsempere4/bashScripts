...
archivoA="/srv/administracioUsuaris/alta.csv"  # El nombre del archivo que dara de alta a esos usuarios
...
IFS=','
while  read -r Operacion Nombre Primer_apellido Login Departamento
do
echo $Departamento
...
EOF
        ...
        search_user=$(ldapsearch -x -LLL -b "uid=$Login,ou=$Departamento,$BASE_DN" uid 2>/dev/null)
        if [ -n "$search_user" ]; then
           if [ "$Operacion" == "Baja" ]; then
                # Eliminar usuario
                ldapdelete -x -D "$ADMIN_DN" -w "$ADMIN_PASS" "uid=$Login,ou=$Departamento,$BASE_DN"
            ...
            if [ "$Operacion" == "Alta" ]; then
                ...
                # Obtener el GID del grupo de departamento
                gid_departamento=$(buscar_gid_departamento "$Departamento")
                # Generar el hash de contraseña
                hashed_password=$(slappasswd -s "Usuario123")
                # Definir LDIF para el usuario
                LDIF_USUARIO=$(cat <<EOF
dn: uid=$Login,ou=$Departamento,$BASE_DN
